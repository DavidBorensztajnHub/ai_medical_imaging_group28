...
hd_val = np.load(BASE + "/hd_val.npy")
...
print(f"Mean Hausdorff (all classes): {np.nanmean(hd_no_bg):.4f}")
...
for c in range(num_classes):
    ...
    print(f"Class {c+1} HD  : {np.nanmean(hd_no_bg[:, :, c]):.4f}")

# Per epoch
print("\n=== Per Epoch ===")
for e in range(dice_no_bg.shape[0]):
    print(f"Epoch {e}: Dice={dice_no_bg[e].mean():.4f}, IoU={iou_no_bg[e].mean():.4f}, HD={np.nanmean(hd_no_bg[e]):.4f}")
...
with open(filename, "w") as f:
    ...
    f.write(f"Mean HD  : {np.nanmean(hd_no_bg):.4f}\n\n")


    for c in range(num_classes):
        f.write(f"Class {c+1} Dice: {dice_no_bg[:, :, c].mean():.4f}\n")
        f.write(f"Class {c+1} IoU : {iou_no_bg[:, :, c].mean():.4f}\n")
        f.write(f"Class {c+1} HD  : {np.nanmean(hd_no_bg[:, :, c]):.4f}\n")

    f.write("\nPer Epoch:\n")
    for e in range(dice_no_bg.shape[0]):
        f.write(f"Epoch {e}: Dice={dice_no_bg[e].mean():.4f}, IoU={iou_no_bg[e].mean():.4f}, HD={np.nanmean(hd_no_bg[e]):.4f}\n")
